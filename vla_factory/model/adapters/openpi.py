...
openpi contract (verified against openpi main):
  ...
  * openpi ``Observation``: ``images`` (dict, **[-1,1] CHW**), ``image_masks``,
    ``state``, ``tokenized_prompt``, ``tokenized_prompt_mask`` — field-for-field
    ~vla ``Observation``; the only difference is image range/layout.
  * ``Pi0Config`` carries **no image_keys** — the resolved assembly supplies the
    slot-to-data camera mapping instead of the adapter re-deriving one.

Image [-1,1] CHW is produced by the resolved transform pipeline; the adapter
does not rescale or change layout.
"""
...
def try_import_openpi():
    ...
    """
    if getattr(try_import_openpi, "_cached", None) is not None:
        return try_import_openpi._cached  # type: ignore[attr-defined]
    try:
        ...
        cached = None
    try_import_openpi._cached = cached  # type: ignore[attr-defined]
    return cached
...
class PI0ModelWrapper(nn.Module):
    ...
    ``camera_mapping`` is the resolved ``{openpi_role: data_camera}`` relation.
    Only mapped roles are populated with real images; unresolved optional slots
    receive placeholders. Images already satisfy the resolved model interface.
    """
    ...
    def _to_openpi_observation(self, observation: Observation):
        _PI0Pytorch, _Pi0Config, OpenpiObservation = try_import_openpi()  # noqa: F841
...
def _resolve_openpi_config(recipe: TrainRecipe, model_name: str) -> TrackedConfig:
    """Return the recipe's resolved model config as a tracked mapping.

    ``merge_model_config()`` already folded the model's declared
...
def load_openpi(
    ...
    """
    cfg = _resolve_openpi_config(recipe, model_name)
    ...
    if recipe.model.path:
        _load_weights(model, recipe.model.path, model_name=model_name)
    else:
    ...
    if not camera_mapping:
        logger.warning(
            "%s: the resolved assembly maps no data camera to an OpenPI slot; "
            "all visual inputs will be placeholders.",
            model_name,
        )

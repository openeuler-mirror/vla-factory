"""Action-fact routing: resolve ``action_spec`` facts from the three
...
- ``action_horizon``→ ModelMetadata for finetune, recipe
                      for from-scratch (R4: ACT-from-scratch legitimately lets
                      the user pick the chunk size; pi0's is family-wide)
...
def resolve_action_horizon(
    *,
    ...
    - ``from_scratch`` (e.g. ACT): the recipe chooses the chunk size → recipe
      is authoritative.
    - ``pretrained_finetune`` (e.g. pi0): the family declaration knows →
      ModelMetadata > recipe.
    """

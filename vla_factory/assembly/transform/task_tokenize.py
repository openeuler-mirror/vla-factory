...
class TaskTokenize(TransformStep):
    ...
    def _ensure_tokenizer(self):
        if self._tokenizer is None:
            ...
            if not repo:
                raise ValueError(
                    "task_tokenize needs a tokenizer source: declare "
                    "ModelMetadata.tokenizer_repo, or point the recipe's "
                    "model.path at a base checkpoint that ships a tokenizer."
                )
        ...
        if task is None:
            ...
            if not self._warned_empty_task:
                ...
                logger.warning(
                    ...
                    "finetuning; set assembly.default_task in the recipe "

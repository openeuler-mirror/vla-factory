...
│   ├── connectors/                  # lightweight callbacks and bootstrap configs imported by remote robot environments
│   ├── platforms/                   # adapters between native platform observations/actions and the unified inference interface
...
The deployment module turns training artifacts into real-time policy services callable by a platform: it rebuilds the training-consistent inference pipeline (model + preprocessor/postprocessor) from a checkpoint, translates each simulator/real-robot platform's native observation into a unified `ObsDict`, runs the model forward, and turns the normalized action chunk back into platform-executable action commands via an execution strategy. It treats the checkpoint's `inference_metadata` (recipe, schema, norm stats) as the single source of truth — it does not rescan the training dataset or re-merge the current code's model profile.

For the full design, see [Deployment Module Design](../modules/deploy-module.md).

Its main responsibilities include:

- Rebuilding the model, preprocessor, and postprocessor from checkpoint metadata.
- Translating each platform's native observation into a unified `ObsDict`.
- Running the model forward and applying the postprocessing inverse transforms.
- Producing actions via the synchronous / temporal_ensembling / receding_horizon execution strategies.
- Serving the engine over ZMQ or a length-prefixed JSON RPC transport, including the dependency-free RoboTwin connector.
...
Automatically fixable issues should be logged as warnings. Issues that break training semantics should fail directly. Whenever the state/action dimension is non-zero, keys must exist and their count must match the dimension exactly. Missing or mismatched keys mean the checkpoint metadata is incomplete and must fail before training writes metadata or when deployment loads the checkpoint. This contract is not relaxed per platform and deployment must not fall back to the live dataset.

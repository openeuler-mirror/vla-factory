...
- **Model IO spec (`ModelIOSpec`)**: what the model actually takes in and gives out once the three descriptions are reconciled — camera keys, state width and prompt requirement on the way in; action width and horizon on the way out. It is the "fact standard" after composition: downstream builds the model against it and reads/writes observation/action by it.
...
4. Plan Pipeline   generate the declarative TransformPipelinePlan
5. Build IO Spec   fold planned calls into observation, action, language, and temporal semantics

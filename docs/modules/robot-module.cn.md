...
`RobotProfile` 当前首先用于描述和校验机器人自身；跨维度检查只消费双方都使用同一受控
词表显式声明的事实，例如 control mode。相机名和关节名处在平台与数据两个命名空间，
没有显式 binding 时不能用字符串相似度证明它们对应。`grippers[]`、关节限位和安全边界
只有在数据/模型侧也具备可对账语义，并且运行时存在真实消费者后，才参与跨维度检查或
生成 transform；在此之前不根据名称和数组位置猜测。

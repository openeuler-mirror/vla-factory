...
本层还持有 `transforms/`（`TransformStep` / `TransformPipeline` / `TransformRegistry` 及各 step 实现），因为组合解析器规划 pipeline、具身组合产出三条 TransformPipelinePlan。
...
- TransformPipelinePlan 的声明式 schema 与 TransformRegistry 能力 metadata（风险、可逆性、inverse）。

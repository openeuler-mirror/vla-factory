...
- Camera ordering: an explicit `camera_names` override takes precedence over
  the schema. The resolved results must be exposed as read-only contract
  fields (`camera_keys` / `state_keys` / `action_keys` / `schema` /
  `recipe`) for upper-layer adapters to build against.
...
- The preprocessor / postprocessor must be rebuilt from — and only from —
  the resolved recipe's `transforms.inputs`; a missing field means the
...
- The output inverse transforms must be generated by the training
  transforms' `inverse_for_output()` (see

...
class TestGoldenRealData:
    ...
    def test_act_mappings_and_plans(self, schema, norm_stats):
        ...
        assert a.state_mapping.entries[0] == {
            ...
            "data_name": "shoulder_pan.pos", "padded": False,
        }
        assert len(a.action_mapping.entries) == 8
        assert not any(e["padded"] for e in a.action_mapping.entries)
    ...
    def test_pi0_pads_both_vectors_to_32(self, schema, norm_stats):
        ...
        # Canonical widths come from the same pad decision the pipeline plans.
        assert a.model_io_spec.state_dim == 32
        assert a.model_io_spec.action_dim == 32
        assert len(a.state_mapping.entries) == 32
        assert sum(1 for e in a.state_mapping.entries if e["padded"]) == 26
        assert sum(1 for e in a.action_mapping.entries if e["padded"]) == 24
...
# ── Equivalence: plan vs the pipeline the build path really constructs ──


class TestPlanMatchesBuiltPipeline:
    """The planned calls must describe the pipeline training actually builds.

    Compared against *built step instances* (attribute by attribute), never by
    feeding the plan back through ``from_config`` — the plan carries resolved
    facts, and ``reject_fact_override`` exists to refuse exactly those as input.
    """

    @staticmethod
    def _built_steps(recipe_path: str):
        from vla_factory.data.formats.lerobot_v3 import LeRobotV3Reader
        ...
        schema = reader.get_schema(DATASET_PATH)
        norm_stats = reader.get_norm_stats(DATASET_PATH)
        metadata = _metadata(recipe.model_name)
        # Mirrors create_dataloaders() / train.py exactly.
        pipeline = _build_transforms(
            norm_stats=norm_stats,
            model_metadata={
                "action_dim": metadata.action_dim or schema.action_dim,
        ...
        return recipe, schema, norm_stats, metadata, pipeline

    @pytest.mark.parametrize("recipe_path", ["examples/act_lekiwi.yaml",
                                             "examples/pi0_lora.yaml"])
    def test_planned_calls_match_built_steps(self, recipe_path):
        if not DATASET_PATH.exists():
            pytest.skip("test dataset not found")
        ...
        recipe, schema, norm_stats, metadata, pipeline = self._built_steps(recipe_path)
        overrides = {
            k: v for k, v in (
                ("camera_mapping", get_camera_mapping(recipe, warn_legacy=False)),
                ("default_task", get_default_task(recipe)),
            ) if v
        }
        plan = resolve_assembly(
            schema, norm_stats, metadata,
            overrides=overrides or None, model_config=recipe.model_config,
        ).data_to_model
...
def _pi0_like() -> ModelMetadata:
    return ModelMetadata(
        name="stub", action_dim=32, dim_policy="padded_to_max", dim_policy_max=32,
...
def test_planner_holds_no_step_names():
    """The structural invariant behind "one planning rule per step": adding a
    transform must never mean editing the resolver. If a step name appears in
    the planner again, the runtime rule has been copied there a second time —
    which is exactly how the planner and ``from_config`` drifted apart before
    (a resize with a non-positive size was accepted by one and refused by the
    other)."""
    ...
    leaked = [name for name in TransformRegistry.names() if f'"{name}"' in code]
    assert leaked == [], f"planner names transform steps: {leaked}"


def test_declared_width_the_pipeline_never_reaches_fails():
    """A model declaring a 32-wide action space but no padding step: the
    pipeline emits the dataset's 8. Reporting either number silently would hand
    downstream a model built for 32 and tensors of 8 — a shape error deep inside
    ...
    err = exc.value.to_dict()
    assert err["code"] == PIPELINE_WIDTH_MISMATCH
    assert err["path"] == "model.action_dim"
    assert err["params"] == {
        "field": "actions", "model_dim": 32,
        "model_dim_source": "metadata", "pipeline_dim": 6,
    }
...
def test_unregistered_step_type_fails_loudly():
    ...
    metadata = ModelMetadata(
        name="stub", requires_prompt=False, vector_normalization="mean_std",
...
def test_inverse_survives_a_step_that_does_not_implement_call_args():
    """The planner and the runtime must agree on whether an inverse exists.
    A step declaring ``inverse_call`` but no ``call_args`` used to plan an
    inverse the runtime then refused to build."""
    from vla_factory.assembly.transforms import TransformRegistry
    from vla_factory.assembly.transforms.base import PlanContext, TransformStep

    @TransformRegistry.register("_probe_no_call_args")
    ...
    try:
        planned = Probe.inverse_call(Probe.compile_call({}, PlanContext()), PlanContext())
        built = Probe.from_config({}).inverse_for_output()
        assert planned == ("unpad_action", {"target_dim": 7})
        assert built is not None and built.target_dim == 7
    finally:
        del TransformRegistry._steps["_probe_no_call_args"]
...
def test_resize_rejects_non_positive_dimensions_like_the_runtime_does():
    """Parity with ``ResizeImages.from_config``: a planner that accepted 0 here
    would emit a resolved plan the pipeline refuses to build."""
    metadata = ModelMetadata(
        name="stub", requires_prompt=False, vector_normalization="mean_std",
        params={"transforms": {"inputs": [
            {"type": "resize_images", "height": 0, "width": 224},
        ]}},
    )
...
def test_unimplementable_normalization_fails_with_a_message_not_a_keyerror():
    ...
    metadata = ModelMetadata(
        name="stub", requires_prompt=False, vector_normalization="min_max",
        params={"transforms": {"inputs": [
...
def test_joint_mapping_embeds_action_names_into_the_robot():
    ...
    assembly = resolve_assembly(
        ...
        ModelMetadata(name="stub", requires_prompt=False, vector_normalization="mean_std"),
        robot_profile=robot,
    )

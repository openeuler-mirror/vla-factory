...
def test_lru_reuses_decoded_frame(dataset):
    ...
    cache = codec._decoders[ref.video_path]["_cache"]
...
def test_lru_eviction(dataset):
    ...
    cache = codec._decoders[refs[0].video_path]["_cache"]
    assert len(cache) == 3
...
def test_lru_key_includes_stream(dataset):
    ...
    cache = codec._decoders[next(iter(refs.values())).video_path]["_cache"]
...
def test_codec_default_max_cached(dataset, tmp_path):
    ...
    caches = [c for c in codec._decoders.values() if c["video_path"].name == "episode0.hdf5"]
    assert len(caches) == 1
    ...
    for ep in range(2):  # fixture lengths 5 + 7 -> 15 / 21 keys per file
        ...
        for frame in ep_frames:
            for ref in frame.images.values():
                codec.decode_frame(ref)
    assert all(len(c["_cache"]) <= codec._max_cached for c in codec._decoders.values())

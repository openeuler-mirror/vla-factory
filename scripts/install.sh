...
install_one_model() {
  ...
  if [[ "$model" == "openvla" ]]; then
    echo ""
    echo "== installing openvla (prismatic + ${VLA_TORCH_BACKEND:-$(detect_cuda_index)} torch) =="

    ensure_openvla_source() {
      local tarball=".local-deps/openvla.tar.gz"
      # Freshness is decided by the pinned REF, not mere directory presence
      # (same policy as openpi/lerobot above).
      if [[ -f "$OPENVLA_DIR/pyproject.toml" && "$(cat "$OPENVLA_DIR/.vla-pin" 2>/dev/null)" == "$OPENVLA_REF" ]]; then
        echo "openvla source present at $OPENVLA_DIR (pin ${OPENVLA_REF:0:8})."
      else
        rm -rf "$OPENVLA_DIR"
        echo "downloading openvla tarball @ ${OPENVLA_REF:0:8}..."
        mkdir -p .local-deps
        curl -fSL \
          --retry 8 --retry-delay 3 --retry-all-errors --retry-connrefused \
          --continue-at - \
          -o "$tarball" \
          "https://github.com/openvla/openvla/archive/${OPENVLA_REF}.tar.gz"
        tar xzf "$tarball" -C .local-deps
        rm -f "$tarball"
        mv ".local-deps/openvla-${OPENVLA_REF}" "$OPENVLA_DIR"
        printf '%s\n' "$OPENVLA_REF" > "$OPENVLA_DIR/.vla-pin"
      fi
    }

    local openvla_pip_flags=(
      --default-index "$UV_DEFAULT_INDEX"
      --torch-backend "${VLA_TORCH_BACKEND:-$(detect_cuda_index)}"
      --index-strategy first-index
      --no-sources-package torch
      --no-sources-package torchvision
    )
    ...
    ensure_openvla_source
    # [vla-factory patch] Defer upstream heavy RLDS/model-registry __init__ chain
    # (tensorflow / dlimp). vla-factory has its own data loader and only needs
    # prismatic.extern.hf + backbones, so neutralize the three eager entrypoints.
    _VLAPATCH='# [vla-factory patch] Heavy RLDS entrypoints deferred (see scripts/install.sh).'
    printf '%s\n' "$_VLAPATCH" > "$OPENVLA_DIR/prismatic/__init__.py"
    printf '%s\n' "$_VLAPATCH" > "$OPENVLA_DIR/prismatic/models/__init__.py"
    printf '%s\n' "$_VLAPATCH" > "$OPENVLA_DIR/prismatic/vla/__init__.py"

    # OpenVLA upstream pyproject pins torch==2.2.0 and pulls tensorflow /
    # tensorflow_datasets / a git dlimp dep (its RLDS data pipeline) — none of
    # which vla-factory needs (it has its own data loader). Install only the
    # minimal model runtime, then prismatic editable with --no-deps.
    retry_uv pip install "${openvla_pip_flags[@]}" ${UV_UPGRADE[@]+"${UV_UPGRADE[@]}"} \
      torch torchvision \
      "transformers==4.40.1" "tokenizers==0.19.1" "timm==0.9.10" \
      accelerate einops "sentencepiece==0.1.99" "peft==0.11.1" rich \
      safetensors huggingface_hub numpy pillow omegaconf pyyaml
    retry_uv pip install "${openvla_pip_flags[@]}" ${UV_UPGRADE[@]+"${UV_UPGRADE[@]}"} --no-deps -e "$OPENVLA_DIR"
    retry_uv pip install "${openvla_pip_flags[@]}" ${UV_UPGRADE[@]+"${UV_UPGRADE[@]}"} -e ".[dev]"
